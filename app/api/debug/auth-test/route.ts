  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return NextResponse.json({
        ...
        received: authHeader ? 'Header exists but not Bearer format' : 'No authorization header'
      }, { status: 401 });
    }
    ...
    // Check admin status
    const isAdmin = payload.isAdmin || payload.role === 'admin' || payload.role === 'superadmin';
